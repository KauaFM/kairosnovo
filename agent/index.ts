...
const AGENT_TOOLS = [
    ...
    },
    {
        ...
        function: {
            name: "create_goal",
            description: "Criar uma meta para o usuário. Use quando ele definir um objetivo, meta ou alvo de longo prazo.",
            parameters: {
                type: "object",
                properties: {
                    title: { type: "string", description: "Título da meta" },
                    description: { type: "string", description: "Descrição detalhada" },
                    category: { type: "string", description: "Categoria: biofisico, cognitivo, financeiro, social, espiritual, digital" },
                    target_value: { type: "number", description: "Valor alvo (opcional)" },
                    deadline: { type: "string", description: "Prazo no formato YYYY-MM-DD (opcional)" }
                },
                required: ["title"]
            }
        }
    },
    {
        ...
        function: {
            ...
            description: "Registrar uma sessão de foco/estudo concluída. Use quando o usuário disser que focou, fez deep work, ou completou um pomodoro.",
            parameters: {
                type: "object",
                properties: {
                    duration_minutes: { type: "integer", description: "Duração em minutos" },
                    notes: { type: "string", description: "O que foi feito durante o foco (opcional)" }
                },
    ...
    },
    {
        ...
        function: {
            name: "save_user_note",
            description: "Salvar uma nota estruturada no banco. Use para anotações importantes, ideias, ou informações que o usuário quer guardar.",
            parameters: {
                type: "object",
                properties: {
                    title: { type: "string", description: "Título da nota (opcional)" },
                    content: { type: "string", description: "Conteúdo da nota" },
                    category: { type: "string", description: "Categoria: geral, ideia, lembrete, insight" }
                },
                required: ["content"]
            }
        }
    ...
    try {
        switch (name) {
            ...
            case "get_today_summary": {
                ...
                const [tasksRes, profileRes, metricsRes, goalsRes, focusRes, activityRes] = await Promise.all([
                    ...
                    supabase.from("goals").select("title,progress,status,deadline").eq("user_id", userId).eq("status", "ativo").catch(() => ({ data: [] })),
                    supabase.from("focus_sessions").select("actual_minutes,duration_minutes,completed").eq("user_id", userId).gte("started_at", today).catch(() => ({ data: [] })),
                    supabase.from("daily_activity").select("*").eq("user_id", userId).eq("activity_date", today).single().catch(() => ({ data: null }))
                ])
                ...
                const dailyAct = activityRes.data
                ...
                const totalFocusMin = focusSessions.filter((s: any) => s.completed).reduce((sum: number, s: any) => sum + (s.actual_minutes || s.duration_minutes || 0), 0)
                ...
                summary += `- Foco hoje: ${totalFocusMin} minutos (${focusSessions.filter((s: any) => s.completed).length} sessões)\n`
                ...
                if (goals.length > 0) {
                    summary += `\nMETAS ATIVAS:\n`
                    goals.forEach((g: any) => {
                        summary += `  • ${g.title}: ${g.progress}% ${g.deadline ? `(prazo: ${g.deadline})` : ''}\n`
                    })
                }

                // Check achievements after summary
                await supabase.rpc("check_achievements", { p_user_id: userId }).catch(() => {})
            ...
            case "create_goal": {
                ...
                await logAction(userId, userPhone, "create_goal", args)
                return `Meta "${args.title}" criada com sucesso!`
            }
            ...
            case "log_focus_session": {
                const { error } = await supabase.from("focus_sessions").insert({
                    ...
                    actual_minutes: args.duration_minutes,
                    completed: true,
                    notes: args.notes || null,
                    started_at: new Date(Date.now() - args.duration_minutes * 60000).toISOString(),
                    ended_at: new Date().toISOString()
                })
                if (error) return `Erro ao registrar foco: ${error.message}`

                // Update profile totals
                await supabase.rpc("update_daily_activity", {
                    p_user_id: userId,
                    p_field: "focus_minutes",
                    p_increment: args.duration_minutes
                })
                await supabase.from("profiles").update({
                    total_focus_minutes: supabase.rpc ? undefined : 0
                }).eq("id", userId)
                // Direct SQL update for total
                await supabase.rpc("update_daily_activity", {
                    p_user_id: userId,
                    p_field: "focus_minutes",
                    p_increment: args.duration_minutes
                ...
                await logAction(userId, userPhone, "log_focus_session", args)
                return `Sessão de foco de ${args.duration_minutes} minutos registrada!`
            }
            ...
            case "save_user_note": {
                ...
                await logAction(userId, userPhone, "save_user_note", args)
