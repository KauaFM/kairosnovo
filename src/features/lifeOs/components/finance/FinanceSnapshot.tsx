// =============================================================
// ORVAX · Life OS — Finance Snapshot (Tela 1)
// =============================================================
import React from 'react';
import { motion } from 'framer-motion';
import { ChevronLeft, Wallet, ArrowDownRight, ArrowUpRight, Minus, ChevronRight } from 'lucide-react';
import { AreaChart, Area, ResponsiveContainer } from 'recharts';
import { AnimatedScore } from '../mind/AnimatedScore';
import type { FinanceScoreData, FinanceDiagnosis } from '../../data/financeTypes';
import { FINANCE_ACCENT, FINANCE_MOCK } from '../../data/financeMockData';
...
interface Props { data: FinanceScoreData; diagnosis: FinanceDiagnosis; onAnalyze: () => void; onBack: () => void; }

export function FinanceSnapshot({ data, diagnosis, onAnalyze, onBack }: Props) {
  ...
  const statusColor = diagnosis.status === 'critical' ? '#ef4444' : diagnosis.status === 'declining' ? '#eab308' : diagnosis.status === 'excellent' ? '#22c55e' : diagnosis.status === 'improving' ? '#22c55e' : '#71717a';
    ...
    <div className={`${PAGE_BG} min-h-screen w-full pb-12`}>
      <header className="px-5 pt-6 pb-2 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: `${FINANCE_ACCENT}15` }}><Wallet size={16} style={{ color: FINANCE_ACCENT }} /></div>
          <h1 className={`text-[18px] font-bold tracking-tight ${T_STRONG}`}>Financeiro</h1>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: statusColor }} />
          <span className={`text-[9px] font-mono tracking-[0.2em] uppercase ${T_MUTED}`}>
            {diagnosis.status === 'critical' ? 'CRÍTICO' : diagnosis.status === 'declining' ? 'EM QUEDA' : diagnosis.status === 'excellent' ? 'EXCELENTE' : diagnosis.status === 'improving' ? 'EVOLUINDO' : 'ESTÁVEL'}
          </span>
        </div>
      </header>
      ...
      <motion.section className="px-5 mt-4" variants={stagger} initial="hidden" animate="show">
        <motion.div variants={fadeUp} className={`${CARD_HERO} p-6 md:p-7`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className={`text-[10px] font-mono tracking-[0.22em] uppercase ${T_LABEL}`}>Score Financeiro</p>
              <div className="mt-3"><AnimatedScore value={score} className="text-[64px] md:text-[80px]" delay={200} /></div>
              {/* Balance sub-value */}
              <p className={`text-[13px] font-mono tabular-nums ${T_LABEL} mt-2`}>Saldo: <span className={`font-bold ${T_STRONG}`}>{fmtBRL(FINANCE_MOCK.balance)}</span></p>
            </div>
          ...
          <motion.div className="mt-5 h-12 -mx-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4, duration: 0.5 }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={sparkData} margin={{ top: 4, right: 0, bottom: 0, left: 0 }}>
                <defs>
                  <linearGradient id="fin-snap-grad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={FINANCE_ACCENT} stopOpacity={0.4} />
                    <stop offset="100%" stopColor={FINANCE_ACCENT} stopOpacity={0} />
                  </linearGradient>
                </defs>
                <Area type="natural" dataKey="v" stroke={FINANCE_ACCENT} strokeWidth={2.5} fill="url(#fin-snap-grad)" isAnimationActive animationDuration={1200} animationBegin={400} dot={false} activeDot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </motion.div>
        </motion.div>
        ...
        <motion.div variants={storyReveal} className="mt-4 px-1">
          <p className={`text-[20px] md:text-[24px] font-bold leading-snug tracking-tight ${T_STRONG}`}>
            {diagnosis.snapshotStory.split(' ').map((word, i) => {
              const isKw = ['cuidado', 'controle', 'crítica', 'crítico', 'saindo'].some(kw => word.toLowerCase().includes(kw));
        ...
        <motion.button type="button" onClick={onAnalyze} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.5, duration: 0.4 }}
          ...
          <span className="flex items-center gap-2"><Wallet size={18} /><span className="text-[13px] font-bold tracking-wide">Analisar em detalhes</span></span>
