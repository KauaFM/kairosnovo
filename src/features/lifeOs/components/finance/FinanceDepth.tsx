// =============================================================
// ORVAX · Life OS — Finance Depth (Tela 3)
...
import type { FinanceIntelData, FinanceDiagnosis } from '../../data/financeTypes';
...
interface Props { data: FinanceIntelData; diagnosis: FinanceDiagnosis; onBack: () => void; }

export function FinanceDepth({ data, diagnosis, onBack }: Props) {
    ...
    <div className={`${PAGE_BG} min-h-screen w-full pb-12`}>
      <header className="px-5 pt-6 pb-2 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-[10px] font-mono tracking-[0.22em] uppercase ${T_LABEL} flex items-center gap-1.5`}><span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: FINANCE_ACCENT }} />Financeiro · Profundidade</p>
      ...
      <div className="px-5 mt-4 space-y-3">
        ...
        <motion.div custom={0} variants={blockReveal} initial="hidden" animate="show"
          ...
          <div className="relative">
            ...
            <p className={`text-[12px] font-mono tracking-wider uppercase ${T_MUTED} mb-1`}>Seu maior desafio financeiro:</p>
            ...
            <div className="mt-5 pt-4 border-t border-rose-200 dark:border-rose-900/40">
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 text-white" style={{ backgroundColor: FINANCE_ACCENT }}><ResolveIcon name={priority.actionIcon} size={18} /></div>
        ...
        <motion.div custom={1} variants={blockReveal} initial="hidden" animate="show" className={`${CARD} p-5`}>
          <SmallHead title="Correlações detectadas" icon={<Compass size={11} style={{ color: FINANCE_ACCENT }} />} />
            ...
            {drivers.filter(d => d.tone === 'negative').slice(0, 3).map((d, i) => (
              <li key={i} className="flex items-center gap-3 p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-200 dark:border-zinc-800">
                ...
                <span className={`text-[14px] font-bold tabular-nums shrink-0 ${d.tone === 'positive' ? 'text-emerald-600' : 'text-rose-600'}`}>
                  {d.tone === 'positive' ? <ArrowUpRight size={11} className="inline" /> : <ArrowDownRight size={11} className="inline" />}
                  {d.impact > 0 ? '+' : ''}{d.impact}%
                </span>
              </li>
            ))}
          ...
          <div className="mt-4 space-y-4">
            {topCorrelations.map((corr, i) => (
              <div key={i}>
                ...
                <ScatterCorrelation data={corr.data} xLabel={corr.xLabel} yLabel={corr.yLabel} accent={FINANCE_ACCENT} pearson={corr.pearson} height={160} />
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div custom={2} variants={blockReveal} initial="hidden" animate="show" className={`${CARD} p-5`}>
          <SmallHead title="Fluxo do dia" sub="Onde o dinheiro vai em 24h" icon={<Clock size={11} style={{ color: FINANCE_ACCENT }} />} />
          <div className="mt-3"><ExecutionTimeline blocks={timeline24h} startHour={7} endHour={23} /></div>
          {patterns.length > 0 && (
            <div className="mt-4 space-y-2">
              <p className={`text-[10px] font-mono tracking-[0.22em] uppercase ${T_LABEL} flex items-center gap-1.5`}><Zap size={10} style={{ color: FINANCE_ACCENT }} />Padrões detectados</p>
              {patterns.map((tp, i) => (
                <div key={i} className={`flex items-center gap-3 p-3 rounded-xl ${tp.type === 'savings_streak' ? 'bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200' : tp.type === 'category_alert' ? 'bg-amber-50 dark:bg-amber-950/20 border border-amber-200' : 'bg-rose-50 dark:bg-rose-950/20 border border-rose-200'}`}>
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-white ${tp.type === 'savings_streak' ? 'bg-emerald-500' : tp.type === 'category_alert' ? 'bg-amber-500' : 'bg-rose-500'}`}>
                    {tp.type === 'savings_streak' ? <Target size={14} /> : <AlertTriangle size={14} />}
                  </div>
        ...
        <motion.div custom={3} variants={blockReveal} initial="hidden" animate="show"
          ...
          <SmallHead title="Previsão · 7 dias" sub="Se nada mudar" icon={<Sparkles size={11} className="text-amber-600" />} />
        ...
        <motion.div custom={4} variants={blockReveal} initial="hidden" animate="show"
          className="rounded-3xl p-5 bg-gradient-to-br from-amber-50 to-white dark:from-amber-950/30 dark:to-zinc-900 border border-amber-200 dark:border-amber-900/50">
          <p className="text-[10px] font-mono tracking-[0.22em] uppercase mb-4 flex items-center gap-1.5" style={{ color: FINANCE_ACCENT }}><Target size={10} />O que fazer agora</p>
          <div className="space-y-2">
            {actions.map((action, i) => (
              <motion.button key={i} type="button" initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 + i * 0.15, duration: 0.3 }}
                className={`w-full flex items-center gap-3 h-12 px-4 rounded-xl text-left bg-white dark:bg-zinc-900 border transition-all ${action.urgency === 'high' ? 'border-amber-300 dark:border-amber-900/50 hover:border-amber-500' : 'border-zinc-200 dark:border-zinc-800'} active:scale-[0.99]`}>
                <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-white" style={{ backgroundColor: action.urgency === 'high' ? FINANCE_ACCENT : '#71717a' }}>
                  <ResolveIcon name={action.icon} size={14} />
                </div>
                <span className={`text-[13px] font-semibold ${T_STRONG}`}>{action.label}</span>
                {action.urgency === 'high' && <span className="ml-auto text-[8px] font-mono tracking-widest uppercase px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300">urgente</span>}
              </motion.button>
            ))}
          </div>
        </motion.div>
      ...
      <p className={`mt-6 px-5 text-center text-[9px] font-mono tracking-[0.22em] uppercase ${T_MUTED}`}>· dinheiro é energia · direcione com intenção ·</p>
    </div>
  );
}
