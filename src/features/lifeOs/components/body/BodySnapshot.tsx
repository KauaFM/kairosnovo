// =============================================================
// ORVAX · Life OS — Body Snapshot (Tela 1)
// Score corpo + diagnóstico + story + micro UX de tensão
// =============================================================
import React from 'react';
import { motion } from 'framer-motion';
import { ChevronLeft, Dumbbell, ArrowDownRight, ArrowUpRight, Minus, ChevronRight } from 'lucide-react';
import { AreaChart, Area, ResponsiveContainer } from 'recharts';
import { AnimatedScore } from '../mind/AnimatedScore';
import type { BodyScoreData, BodyDiagnosis } from '../../data/bodyTypes';
...
export function BodySnapshot({ data, diagnosis, onAnalyze, onBack }: Props) {
  ...
  return (
    <div className={`${PAGE_BG} min-h-screen w-full pb-12`}>
      <header className="px-5 pt-6 pb-2 flex items-center gap-3">
        <button type="button" onClick={onBack} className={`w-10 h-10 -ml-1 rounded-full flex items-center justify-center shrink-0 ${T_LABEL} bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors`} aria-label="Voltar">
          <ChevronLeft size={20} strokeWidth={2.2} />
        </button>
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: `${BODY_ACCENT}15` }}>
            <Dumbbell size={16} style={{ color: BODY_ACCENT }} />
          </div>
          <h1 className={`text-[18px] font-bold tracking-tight ${T_STRONG}`}>Corpo</h1>
        </div>
      ...
      <motion.section className="px-5 mt-4" variants={stagger} initial="hidden" animate="show">
        <motion.div variants={fadeUp} className={`${CARD_HERO} p-6 md:p-7`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className={`text-[10px] font-mono tracking-[0.22em] uppercase ${T_LABEL}`}>Score Físico</p>
              <div className="mt-3">
                <AnimatedScore value={score} className="text-[64px] md:text-[80px]" delay={200} />
              </div>
            </div>
          ...
          <motion.div className="mt-5 h-12 -mx-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4, duration: 0.5 }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={sparkData} margin={{ top: 4, right: 0, bottom: 0, left: 0 }}>
                <defs>
                  <linearGradient id="body-snap-grad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={BODY_ACCENT} stopOpacity={0.4} />
                    <stop offset="100%" stopColor={BODY_ACCENT} stopOpacity={0} />
                  </linearGradient>
                </defs>
                <Area type="natural" dataKey="v" stroke={BODY_ACCENT} strokeWidth={2.5} fill="url(#body-snap-grad)" isAnimationActive animationDuration={1200} animationBegin={400} dot={false} activeDot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </motion.div>
        </motion.div>
        ...
        <motion.div variants={tensionReveal} className={`${CARD_HERO} p-5 mt-3`} style={{ borderLeft: `3px solid ${statusColor}` }}>
          <p className={`text-[9px] font-mono tracking-[0.22em] uppercase ${T_MUTED} mb-2 flex items-center gap-1.5`}>
            <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: statusColor }} />Diagnóstico
          </p>
        ...
        <motion.div variants={storyReveal} className="mt-4 px-1">
          <p className={`text-[20px] md:text-[24px] font-bold leading-snug tracking-tight ${T_STRONG}`}>
            {diagnosis.snapshotStory.split(' ').map((word, i) => {
              const isKw = ['queda', 'caindo', 'crítico', 'insuficiente'].some(kw => word.toLowerCase().includes(kw));
