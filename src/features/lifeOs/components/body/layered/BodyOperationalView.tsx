// =============================================================
// ORVAX · BodyOperationalView (Camada 1 · Saúde) — DIA + SEMANA.
// Estrutura idêntica a MindOperationalView, dados do BODY_MOCK.
// =============================================================
import React, { useMemo } from 'react';
import { motion } from 'framer-motion';
import * as Icons from 'lucide-react';
import {
  ChevronLeft, ChevronRight, Heart, Clock, Calendar,
...
function statusToLight(status: BodyDiagnosis['status']) {
...
export function BodyOperationalView({ data, diagnosis, onStrategic, onBack }: Props) {
      ...
      <header className="sticky top-0 z-10 backdrop-blur-md bg-zinc-50/80 dark:bg-zinc-950/80
        ...
        <div className="flex-1 min-w-0">
          ...
          <h1 className={`text-[15px] font-bold leading-tight truncate ${T_STRONG}`}>
            <Heart size={14} className="inline -mt-0.5 mr-1.5 text-emerald-500" />
            Saúde · agora
          </h1>
        </div>
      ...
      <div className="px-5 pt-4 space-y-4">
              ...
              <div className="flex-1 h-16 -mr-2">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data.snapshot.sparkline7d.map((v, i) => ({ i, v }))}
                    ...
                    <defs>
                      <linearGradient id="bop-spark" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Area type="monotone" dataKey="v" stroke={EMERALD} strokeWidth={2.5}
                      fill="url(#bop-spark)" dot={false} isAnimationActive />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        <motion.div custom={1} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            <SectionHead
              ...
              sub="treino · sono · alimentação · descanso"
            />
            <div className="mt-3">
            ...
            <InsightStrip
              ...
              text={
                drainHours > 2
                  ? `${drainHours.toFixed(1)}h de drenagem hoje (sedentarismo / mau sono) · ${focusHours.toFixed(1)}h de execução produtiva`
        ...
        <motion.div custom={2} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            <SectionHead
              ...
              title="Estado atual"
              sub="força · resistência · sono · nutrição · recuperação"
            />
        ...
        <motion.div custom={3} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            <SectionHead
              ...
              sub="evolução do score corporal"
            />
            <div className="mt-2 h-[120px]">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  <defs>
                    <linearGradient id="bop-week-grad" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="score" stroke={EMERALD} strokeWidth={2.4}
                    fill="url(#bop-week-grad)" dot={{ r: 3, stroke: EMERALD, strokeWidth: 2, fill: '#fff' }}
                    isAnimationActive />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            <InsightStrip
              ...
              text={
                data.snapshot.trend7d < -5
                  ? `Você perdeu consistência física · queda de ${Math.abs(data.snapshot.trend7d)}% em 7 dias`
        ...
        <motion.div custom={6} initial="hidden" animate="show" variants={blockReveal}>
          ...
          <p className={`mt-2 text-center text-[10px] font-mono tracking-wider ${T_MUTED}`}>
            mês + ano · pra onde sua saúde está indo
          </p>
        </motion.div>
