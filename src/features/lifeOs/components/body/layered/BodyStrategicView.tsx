// =============================================================
// ORVAX · BodyStrategicView (Camada 2 · Saúde) — MÊS + ANO.
// Isolamento total · zero leak de outros pilares.
//
// Blocos:
//   1. Evolução 30d (workoutSeries)
//   2. Mapa interno · Saúde · ghost layer (atual vs 30d atrás)
//   3. Foco da área · alavancagem cruzada (texto, sem chart)
//   4. Padrões de vida · top correlation
//   5. Previsão (próximos 30d)
//   6. A Verdade
//   7. Direção (1 prioridade + 1 hábito)
// =============================================================
import React, { useMemo } from 'react';
import { motion } from 'framer-motion';
import {
  ChevronLeft, TrendingUp, Compass, Heart, Hexagon,
...
// Saúde · áreas que ela impacta diretamente.
// Texto puro, sem chart. Mantém o isolamento — não puxa global radar.
const BODY_IMPACTS = [
  { label: 'Mente',         sub: 'foco · clareza · ansiedade · sono é cognição',     strength: 'forte'  as const },
  { label: 'Bem-estar',     sub: 'humor · energia · disposição diária',              strength: 'forte'  as const },
  { label: 'Produtividade', sub: 'capacidade física pra deep work prolongado',       strength: 'média'  as const },
];
...
interface Props {
  data: BodyData;
  diagnosis: BodyDiagnosis;
  onOperational: () => void;
  onBack: () => void;
}

export function BodyStrategicView({ data, diagnosis, onOperational, onBack }: Props) {
  // Evolução 30d · uso da workoutSeries como métrica primária
  const monthDelta = useMemo(() => {
    const series = data.workoutSeries30d;
    const first = series[0]?.value ?? 0;
    const last  = series[series.length - 1]?.value ?? 0;
    return Math.round(((last - first) / Math.max(1, first)) * 100);
  }, [data.workoutSeries30d]);

  // Radar interno da Saúde · 5 eixos com ghost layer (atual vs 30d atrás)
  const bodyRadar = useMemo(() => data.radar.map(r => ({
    axis:     r.axis,
    atual:    r.current,
    anterior: r.previous,
  })), [data.radar]);

  // Top correlation · do diagnóstico (top 2 por |pearson|), fallback pra correlations cruas
  ...
  return (
    <div className={`min-h-screen ${PAGE_BG} pb-24`}>
      ...
      <div className="px-5 pt-4 space-y-4">
        ...
        <motion.div custom={0} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD, 'p-5'].join(' ')}>
            <SectionHead
              ...
              sub="trajetória do score corporal"
            />
            ...
            <p className={`mt-1 text-[13px] leading-relaxed ${T_NORMAL}`}>
              {monthDelta >= 0
                ...
                : `Queda de ${Math.abs(monthDelta)}% no último mês · curva de regressão física`}
            </p>
            <div className="mt-3 h-[180px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.workoutSeries30d} margin={{ top: 6, right: 8, bottom: 0, left: -20 }}>
                  <defs>
                    <linearGradient id="bstrat-30d" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="value" stroke={EMERALD} strokeWidth={2.4}
                    fill="url(#bstrat-30d)" dot={false} isAnimationActive />
        ...
        {/* ── BLOCO 2 · MAPA INTERNO DA SAÚDE · ghost layer ──── */}
        <motion.div custom={1} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            <SectionHead
              ...
              title="Mapa interno · Saúde"
              sub="atual vs 30 dias atrás · ghost layer"
            />
            <div className="mt-2 h-[260px]">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={bodyRadar} outerRadius="70%">
            ...
            <p className={`mt-3 text-[11.5px] leading-relaxed ${T_NORMAL}`}>
              Sono caiu <strong>7pts</strong> em 30 dias — único eixo regredindo enquanto Força e Nutrição evoluíram.
              É o vetor que está puxando a média pra baixo.
            </p>
          </div>
        </motion.div>
        ...
        <motion.div custom={1.5} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            ...
            <p className={`mt-3 text-[11px] leading-relaxed ${T_NORMAL}`}>
              Saúde é base · subir aqui alavanca esses pilares juntos sem esforço cognitivo extra.
            </p>
          </div>
        </motion.div>
          ...
          <motion.div custom={2} initial="hidden" animate="show" variants={blockReveal}>
            <div className={[CARD_SUB, 'p-5'].join(' ')}>
              ...
              <p className={`mt-2 text-[12px] leading-relaxed ${T_NORMAL}`}>
                Seu desempenho corporal depende diretamente de <strong>{topCorrelation.xLabel.toLowerCase()}</strong>.
              </p>
            </div>
          </motion.div>
        )}
          ...
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              {diagnosis.predictions.slice(0, 4).map((p, i) => {
                ...
                const lower = p.metric.toLowerCase();
                // Métricas onde subir é bom: força, sono, recuperação, energia
                const goodIfUp = /for[çc]a|sono|recupera|energia|nutri|treino/i.test(lower);
