// =============================================================
// ORVAX · StrategicView (Camada 2) — MÊS + ANO · longo prazo.
// Responde "pra onde minha vida está indo?" → direção de vida.
//
// Blocos:
//   1. Evolução 30d (linha longa)
//   2. Mapa de Equilíbrio (radar de TODOS os pilares · contexto sistêmico)
//   3. Padrões de Vida (correlations top 2 · pearson)
//   4. Fluxo de Vida (Sankey · tempo → comportamento → resultado)
//   5. Previsão (predictions next 30d)
//   6. Verdade (impact text gigante)
//   7. Direção (1 foco + 1 hábito)
//   + Botão "Voltar à visão operacional"
// =============================================================
import React, { useMemo } from 'react';
import { motion } from 'framer-motion';
import {
  ChevronLeft, TrendingUp, Compass, Brain, Hexagon,
...
// =============================================================
// Mente · áreas que ela impacta diretamente.
// Em vez de um "mapa global da vida" (esse vive só na HOME),
// listamos quais OUTROS pilares são alavancados quando a Mente sobe.
// =============================================================
const MIND_IMPACTS = [
  { label: 'Produtividade', sub: 'foco · execução · deep work',           strength: 'forte' as const },
  { label: 'Bem-estar',     sub: 'humor · ansiedade · oscilação diária',  strength: 'forte' as const },
  { label: 'Relacionamentos', sub: 'presença · paciência · empatia',      strength: 'média' as const },
];
...
export function StrategicView({ data, diagnosis, onOperational, onBack }: Props) {
  // Evolução 30d · % desde início
  const monthDelta = useMemo(() => {
    const first = data.scoreSeries30d[0]?.score ?? 0;
    const last  = data.scoreSeries30d[data.scoreSeries30d.length - 1]?.score ?? 0;
    return Math.round(((last - first) / Math.max(1, first)) * 100);
  }, [data.scoreSeries30d]);

  // Radar interno da Mente · 5 eixos com camada-fantasma (atual vs 30d atrás).
  // "Calma" já é o inverso de ansiedade no mock — alto = bom.
  const mindRadar = useMemo(() => data.radar.map(r => ({
    axis:    r.axis,
    atual:   r.current,
    anterior: r.previous,
  })), [data.radar]);
  ...
  // Top correlation
  const topCorrelation = data.correlations[0];
  ...
  return (
    <div className={`min-h-screen ${PAGE_BG} pb-24`}>
      ...
      <div className="px-5 pt-4 space-y-4">
        ...
        <motion.div custom={0} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD, 'p-5'].join(' ')}>
            <SectionHead
              ...
              sub="trajetória do score mental"
            />
            ...
            <div className="mt-3 h-[180px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.scoreSeries30d} margin={{ top: 6, right: 8, bottom: 0, left: -20 }}>
                  <defs>
                    <linearGradient id="strat-30d" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Tooltip
                    ...
                    formatter={(v: number, k: string) =>
                      [v, k === 'score' ? 'Score' : 'Média 7d']}
                  />
                  <Area type="monotone" dataKey="score" stroke={EMERALD} strokeWidth={2.4}
                    fill="url(#strat-30d)" dot={false} isAnimationActive />
        ...
        {/* ── BLOCO 2 · MAPA INTERNO DA MENTE · ghost layer ──── */}
        <motion.div custom={1} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            <SectionHead
              ...
              title="Mapa interno · Mente"
              sub="atual vs 30 dias atrás · ghost layer"
            />
            <div className="mt-2 h-[260px]">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={mindRadar} outerRadius="70%">
                  ...
                  {/* Anterior · ghost · tracejado neutro */}
                  <Radar
                    name="30d atrás"
                    dataKey="anterior"
                    stroke="#a1a1aa"
            ...
            <p className={`mt-3 text-[11.5px] leading-relaxed ${T_NORMAL}`}>
              Foco caiu <strong>13pts</strong> em 30 dias · Calma e Energia Cognitiva também regrediram.
              Clareza foi o único eixo que evoluiu.
            </p>
          </div>
        </motion.div>
        ...
        <motion.div custom={1.5} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            ...
            <p className={`mt-3 text-[11px] leading-relaxed ${T_NORMAL}`}>
              Subir a Mente puxa esses pilares juntos. É a alavanca de maior efeito sistêmico hoje.
            </p>
          </div>
        </motion.div>
          ...
          <motion.div custom={2} initial="hidden" animate="show" variants={blockReveal}>
            <div className={[CARD_SUB, 'p-5'].join(' ')}>
              ...
              <div className="mt-2 h-[160px]">
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 6, right: 6, bottom: 4, left: -12 }}>
                    ...
                    <XAxis
                      type="number" dataKey="x" name={topCorrelation.xLabel}
                      tick={{ fontSize: 9, fontFamily: 'ui-monospace, monospace', fill: '#71717a' }}
                      axisLine={false} tickLine={false}
                    />
                    <YAxis
                      type="number" dataKey="y" name={topCorrelation.yLabel}
                      tick={{ fontSize: 9, fontFamily: 'ui-monospace, monospace', fill: '#71717a' }}
                      axisLine={false} tickLine={false}
                    />
                    <Tooltip
              ...
              <p className={`mt-2 text-[12px] leading-relaxed ${T_NORMAL}`}>
                Seu desempenho mental depende diretamente de <strong>{topCorrelation.xLabel.toLowerCase()}</strong>.
                Mexer aqui muda tudo em cascata.
              </p>
            </div>
          </motion.div>
        )}

        {/* ── BLOCO 4 · FLUXO DE VIDA (SANKEY) ────────────────── */}
          ...
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              {diagnosis.predictions.slice(0, 4).map((p, i) => {
                ...
                const isPositive = (p.metric.toLowerCase().includes('foco') || p.metric.toLowerCase().includes('clareza')) ? isUp : !isUp;
                return (
                  <div key={i} className={[
        ...
        {/* ── footer · botão voltar pra operacional ───────────── */}
