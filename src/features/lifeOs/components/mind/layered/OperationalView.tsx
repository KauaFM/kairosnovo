// =============================================================
// ORVAX · OperationalView (Camada 1) — DIA + SEMANA · curto prazo.
// Responde "o que tá acontecendo hoje?" → tomada de decisão imediata.
//
// Blocos:
//   1. Status atual (top): score + traffic light + storytelling
//   2. Hoje (micro): ExecutionTimeline 24h + insight + mini Radar 4 eixos
//   3. Semana: line 7d + heatmap semanal + insight
//   4. Problema Principal (auto): diagnosis.priority
//   5. Ação Imediata: 3 botões smart de diagnosis.actions
//   + CTA: "Ver visão estratégica"
// =============================================================
import React, { useMemo } from 'react';
import { motion } from 'framer-motion';
import * as Icons from 'lucide-react';
import {
  ChevronLeft, ChevronRight, Brain, Clock, Calendar,
...
// Traffic light por status do diagnóstico
function statusToLight(status: MindDiagnosis['status']) {
  if (status === 'excellent' || status === 'improving') {
    return { dot: 'bg-emerald-500', label: 'BOM', tone: 'good' as const };
  }
  if (status === 'stable') {
    return { dot: 'bg-amber-500', label: 'ATENÇÃO', tone: 'attention' as const };
  }
  return { dot: 'bg-rose-500', label: 'CRÍTICO', tone: 'critical' as const };
}
...
export function OperationalView({ data, diagnosis, onStrategic, onBack }: Props) {
  const light = statusToLight(diagnosis.status);

  // Dados do dia · derivado do timeline 24h
  const drainHours = useMemo(() => {
    return data.timeline24h
      ...
      <header className="sticky top-0 z-10 backdrop-blur-md bg-zinc-50/80 dark:bg-zinc-950/80
        ...
        <div className="flex-1 min-w-0">
          ...
          <h1 className={`text-[15px] font-bold leading-tight truncate ${T_STRONG}`}>
            <Brain size={14} className="inline -mt-0.5 mr-1.5 text-emerald-500" />
            Mente · agora
          </h1>
        </div>
      ...
      <div className="px-5 pt-4 space-y-4">
              ...
              <div className="flex-1 h-16 -mr-2">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data.snapshot.sparkline7d.map((v, i) => ({ i, v }))}
                    ...
                    <defs>
                      <linearGradient id="op-spark" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Area type="monotone" dataKey="v" stroke={EMERALD} strokeWidth={2.5}
                      fill="url(#op-spark)" dot={false} isAnimationActive />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        <motion.div custom={1} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            <SectionHead
              ...
              sub="distribuição do dia"
            />
            <div className="mt-3">
              <ExecutionTimeline blocks={data.timeline24h} startHour={6} endHour={24} />
            </div>
            <InsightStrip
              icon={drainHours > 2 ? <AlertTriangle size={11} className="text-rose-500" /> : <Sparkles size={11} className="text-emerald-500" />}
              tone={drainHours > 2 ? 'warning' : 'good'}
              text={
                drainHours > 2
                  ? `Você perdeu ${drainHours.toFixed(1)}h em distração hoje · ${focusHours.toFixed(1)}h de foco real`
                  : `Dia limpo · ${focusHours.toFixed(1)}h de foco real e apenas ${drainHours.toFixed(1)}h de drain`
              }
            />
          </div>
        </motion.div>
        ...
        <motion.div custom={2} initial="hidden" animate="show" variants={blockReveal}>
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            <SectionHead
              ...
              title="Estado atual"
              sub="foco · clareza · ansiedade · energia"
            />
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart
                  data={data.snapshot.sparkline7d.map((v, i) => {
                    ...
                    return { day: `${d.getDate().toString().padStart(2,'0')}/${(d.getMonth()+1).toString().padStart(2,'0')}`, score: v };
                  })}
                  margin={{ top: 6, right: 6, bottom: 0, left: -20 }}
                >
                  <defs>
                    <linearGradient id="op-week-grad" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="score" stroke={EMERALD} strokeWidth={2.4}
                    fill="url(#op-week-grad)" dot={{ r: 3, stroke: EMERALD, strokeWidth: 2, fill: '#fff' }}
                    isAnimationActive />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className={[CARD_SUB, 'p-5'].join(' ')}>
            ...
            <div className="mt-3 space-y-2">
              {diagnosis.actions.slice(0, 3).map((a, i) => {
                const ActionIcon = (Icons as unknown as Record<string, React.ComponentType<{ size?: number; strokeWidth?: number; className?: string }>>)[a.icon] || Icons.Zap;
                return (
                  <button
                    key={i}
                    type="button"
        ...
        <motion.div custom={6} initial="hidden" animate="show" variants={blockReveal}>
          ...
          <p className={`mt-2 text-center text-[10px] font-mono tracking-wider ${T_MUTED}`}>
            mês + ano · pra onde sua vida está indo
          </p>
        </motion.div>
