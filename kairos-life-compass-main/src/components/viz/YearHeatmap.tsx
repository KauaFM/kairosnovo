/**
 * Execution map estilo GitHub — 365 dias em colunas semanais.
 */
...
export function YearHeatmap({ cells }: { cells: Cell[] }) {
  // Empacotar em 53 colunas × 7 linhas
  ...
  return (
    <div className="space-y-3">
              ...
              {col.map((cell, ri) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      backgroundColor: lvl === 0 ? "var(--surface-2)" : "var(--success)",
                      opacity: lvl === 0 ? 0.6 : OPACITY[lvl],
                      animation: `fade-in 0.4s ease-out ${(ci * 7 + ri) * 1.5}ms backwards`,
                    }}
                  />
                );
              })}
      ...
      <div className="flex items-center justify-between label-meta">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {[0, 1, 2, 3, 4].map((l) => (
            <div key={l} className="w-[10px] h-[10px] rounded-[2px]"
              style={{
                backgroundColor: l === 0 ? "var(--surface-2)" : "var(--success)",
