import type { SankeyLink } from "@/lib/data-engine";
...
export function Sankey({ nodes, links }: { nodes: string[]; links: SankeyLink[] }) {
  ...
  const maxVal = Math.max(...nodes.map(totalIn));
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto">
      {links.map((l, i) => {
        ...
        const tx = t.x;
        const cx = (sx + tx) / 2;
        const stroke = l.target.includes("+") || l.source === "Foco" ? "var(--success)" : l.target.includes("−") || l.source === "Distração" ? "var(--destructive)" : "var(--pillar)";
        return (
          <path
            key={i}
            ...
            fill="none"
            opacity={0.35}
            style={{ animation: `fade-in 0.6s ease-out ${i * 120}ms backwards` }}
          />
        );
      })}
      {Object.entries(nodePos).map(([n, p]) => (
        <g key={n}>
          <rect x={p.x} y={p.y} width={nodeW} height={p.height} rx={3}
            fill={n.includes("+") ? "var(--success)" : n.includes("−") ? "var(--destructive)" : "var(--pillar)"} />
