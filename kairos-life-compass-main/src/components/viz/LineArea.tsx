import type { DayPoint } from "@/lib/data-engine";
...
export function LineArea({ data, height = 180 }: { data: DayPoint[]; height?: number }) {
  ...
  const pts = data.map((d, i) => [padding + i * step, padding + (h - padding * 2) - ((d.value - min) / range) * (h - padding * 2)] as const);
  const line = pts.map(([x, y], i) => `${i === 0 ? "M" : "L"}${x},${y}`).join(" ");
  const area = `${line} L${pts[pts.length - 1][0]},${h - padding} L${padding},${h - padding} Z`;
